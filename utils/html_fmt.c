...
int main(void)
{
	...
	for (int i = 0; i < input_length && (input[i] == ' ' || input[i] == '\t'); ++i) {
		put(input[i]);
	...
	while (word_start < input_length) {
		if (need_space && 1 + cur_width + word_fmtlength > WIDTH) {
			...
			need_space = true;
			for (int i = 0; i < word_datalength; ++i)
				put(input[word_start + i]);
